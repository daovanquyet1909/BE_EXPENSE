...
exports.getExpenses = async (req, res) => {
    try {
        const expenses = await Expense.find({ user_id: req.user._id }).populate('category_id wallet_id');
        res.json(expenses);
    } catch (error) {
        res.status(400).json({ error: error.message });
    }
...
exports.getExpenseById = async (req, res) => {
    try {
        const expense = await Expense.findById(req.params.id).populate('category_id wallet_id');
        if (!expense) {
